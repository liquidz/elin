...
function! s:vim_get_top_list(lnum, col) abort
  call cursor(a:lnum, a:col)
...
function! s:vim_get_list(lnum, col) abort
  call cursor(a:lnum, a:col)
...
if has('nvim') && exists('*nvim_get_runtime_file') && len(nvim_get_runtime_file('parser', v:true)) > 0
  ...
  function! s:get_top_list(lnum, col) abort
    try
      let res = luaeval('require("elin.sexpr").get_top_list(_A[1], _A[2])', [a:lnum - 1, a:col - 1])
      if type(res) != v:t_dict
        return s:vim_get_top_list(a:lnum, a:col)
      endif
      return res
    catch
      return s:vim_get_list(a:lnum, a:col)
    endtry
  endfunction

  function! s:get_list(lnum, col) abort
    try
      let res = luaeval('require("elin.sexpr").get_list(_A[1], _A[2])', [a:lnum - 1, a:col - 1])
      if type(res) != v:t_dict
        return s:vim_get_list(a:lnum, a:col)
      endif
      return res
