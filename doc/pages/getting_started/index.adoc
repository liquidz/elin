...
==== (OPTIONAL) Enabling elin command [[elin-command]]

`elin` is a utility command for Elin.
To setting up, just add the `bin` directory under the installed elin directory to `$PATH` environmental variable.

[source,shell]
----
# For bash or zsh
$ export PATH=$PATH:/path/to/elin/bin

# For fish
$ set -x PATH /path/to/elin/bin $PATH
----
...
[source,shell]
----
$ clj -Sdeps '{:deps {nrepl/nrepl {:mvn/version "1.5.2"} cider/cider-nrepl {:mvn/version "0.58.0"}}}' -M -m nrepl.cmdline --middleware "[cider.nrepl/cider-middleware]" --interactive
----
